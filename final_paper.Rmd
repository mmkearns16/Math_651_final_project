...
#Histogram of original count data
hist(Olympic_v2$count, lwd = 4, main = 'Untransformed', xlab = 'Medal Count')

#log(count)
hist(log(Olympic_v2$count),main = "Transformed",xlab = "log(Medal Count)")
```
...
```{r, echo=FALSE, fig.pos='h', out.extra = '', fig.cap='Final Linear Regression Residual Analysis'}
par(mfrow=c(2,2))
plot(olympic.lmfinal)
```
...
```{r AIC_comp, echo = F}
df_20<-data.frame(matrix(c(2008.82, 0.751, 'TBD', 2319.23,  0.822, 'TBD'), nrow = 2, byrow = T))
...
par(mfrow=c(1,2))
plot(x=exp(Olympic_v2$log_count), y = exp(olympic.lmfinal$fitted.values), xlim=c(0,120),ylim=c(0,270),col='blue', pch=20, xlab='Actual',
     ylab='Predicted', main='OLS Linear Regression')
abline(a=0,b=1, col='red')
plot(x=exp(Olympic_v2$log_count), y = olympic.nb$fitted.values, col='blue', xlim=c(0,120), ylim=c(0,270),pch=20, xlab='Actual',

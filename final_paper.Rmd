...
We generate a correlation matrix (Table 3) and paired scatterplot (Figure 3) to illustrate the relationships between the variables, especially any multicollinearity. 
...
According to the Normal Q-Q plot (Figure 4), it is reasonable to assume the residuals follow a normal distribution. However, the Residuals vs Fitted plot indicates possible heteroskedasticity or non-constant variance of the errors. The Breusch-Pagan test will help us determine if heteroskedasticity is affecting the model.
...
A negative binomial regression model assumes that dispersion is greater than 1, which is consistent with these data. Therefore, it allows for more accurate tests of the parameters. Among the negative binomial models, the best model again proved to be the full model, after a comparison of AIC between all subsets of variables (Table 5).
...
The same analysis on the negative binomial model showed that a similar number of the 90% predictions intervals for 2016 fell around their observed intervals.
